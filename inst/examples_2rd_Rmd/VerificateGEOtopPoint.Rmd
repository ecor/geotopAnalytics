...
inputPanel(
  ...
  selectInput(inputId = "variable", label = "discover variable", choices = obsnames,selected=obsnames[1]),
  
  selectInput(inputId = "add_variable", label = "additional variable (sim)", choices = simnames, selected = simnames[1])
...
renderDygraph({
  ...
  ## TO GO ON .....
    ...
    data <- extractGeotopVar(x=alldata,InputVar=iiv,Add_InputVar=aiiv) 
    unit <- attr(alldata,"var_unit")[iiv]
    add_unit <- attr(alldata,"var_unit")[aiiv]
    ## 
    
    dygraph(data, ylab=paste("[",unit,"]",sep="")) %>%
      dyRangeSelector() %>%
      dyRoller() %>%
      dySeries(name = "additional var", axis = "y2", stepPlot = TRUE, fillGraph = TRUE, label = paste("[-",add_unit,"]",sep=""))
