...
renderDataTable({
 if (input$aggregation=="hour" | input$aggregation=="day")
  ...
  gofs <- gofg(sim = data$simulation, obs=data$observation, na.rm = T)
  ...
  gofs$DJF <-  c(gofg(sim = datadjf$simulation, obs=datadjf$observation, na.rm = T))
  datamam <-  data[mon==3 | mon==4 | mon==5,]
  gofs$MAM <-  c(gofg(sim = datamam$simulation, obs=datamam$observation, na.rm = T))
  datajja <-  data[mon==6 | mon==7 | mon==8,]
  gofs$JJA <-  c(gofg(sim = datajja$simulation, obs=datajja$observation, na.rm = T))
  datason <-  data[mon==9 | mon==10 | mon==11,]
  gofs$SON <-  c(gofg(sim = datason$simulation, obs=datason$observation, na.rm = T))
